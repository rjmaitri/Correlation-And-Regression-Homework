...
#### Arcsine of the unbound fraction might be useful for linearity.

```{r}


ggplot(data = liver_data,
       mapping = aes(x = concentration, y = asin(unboundFraction))) +
  geom_point(colour = "Orange")+
  stat_smooth(method =lm, fill = "lightblue", colour = "brown")+
  theme_dark()
...
liver_lm <- lm(asin(unboundFraction) ~ concentration, data = liver_data)
...
<span style="color: green;">The residuals vs. fitted aren't a nice cloud shape
...
#### 4d. Using predict() and geom_ribbon() in ggplot2, reproduce the above plot showing data, fit, fit interval, and prediction interval.
...
#### 4c. What pair of lines show the prediction interval? What do these confidence interval bands tell us?
